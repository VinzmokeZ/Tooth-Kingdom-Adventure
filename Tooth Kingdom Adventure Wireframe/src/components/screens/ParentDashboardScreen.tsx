...
// ─── HOME TAB ──────────────────────────────────────────────────────
function ParentHomeTab({ userData }: any) {
    ...
    return (
        <div className="space-y-6">
            {/* Brushing History */}
            <div>
                <h3 className="font-black text-gray-400 tracking-widest uppercase text-[10px] mb-4 px-1">Brushing History</h3>
                <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden divide-y divide-gray-50">
                    {recentActivity.length > 0 ? recentActivity.map((log: any) => (
                        <div key={log.date} className="p-5 flex items-center justify-between">
                            <div>
            ...
            <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white rounded-[2rem] p-5 shadow-xl border border-gray-100 relative overflow-hidden">
                        <div className="absolute -right-3 -top-3 w-14 h-14 bg-purple-100 rounded-full opacity-60" />
                        <p className="text-[9px] font-black text-purple-600 uppercase tracking-widest mb-1">Current Level</p>
                        <p className="text-4xl font-black text-gray-900">{userData.level}</p>
                        <p className="text-[9px] text-gray-400 font-bold mt-1">{userData.xp} XP earned</p>
                        <TrendingUp className="absolute right-4 bottom-4 w-6 h-6 text-purple-200" />
                    </div>
                    <div className="bg-white rounded-[2rem] p-5 shadow-xl border border-gray-100 relative overflow-hidden">
                        <div className="absolute -right-3 -top-3 w-14 h-14 bg-amber-100 rounded-full opacity-60" />
                        <p className="text-[9px] font-black text-amber-600 uppercase tracking-widest mb-1">Total Stars</p>
                        <p className="text-4xl font-black text-gray-900">{userData.totalStars}</p>
                        <p className="text-[9px] text-gray-400 font-bold mt-1">All-time earned</p>
                        <Star className="absolute right-4 bottom-4 w-6 h-6 text-amber-200" />
                    </div>
                </div>
            </div>
            ...
            {/* Streak card */}
            <div className="rounded-[2.5rem] p-5 shadow-xl relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #FFB86C 0%, #FF6AC1 100%)' }}>
                ...
                <div className="relative z-10">
                    ...
                    <div className="flex items-center gap-5 mt-4">
                        <div className="w-16 h-16 rounded-full flex items-center justify-center border-[3px] shadow-sm" style={{ borderColor: 'rgba(255,255,255,0.6)' }}>
...
// ─── PROGRESS TAB ──────────────────────────────────────────────────
function ParentProgressTab({ userData }: any) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden divide-y divide-gray-50">
                    {[
                        { icon: '🦷', title: 'First Brush', desc: 'Completed their first lesson', done: true },
                        { icon: '🔥', title: '7-Day Streak', desc: 'Brushed 7 days in a row', done: userData.currentStreak >= 7 },
                        { icon: '⭐', title: '100 Stars', desc: 'Earned 100 stars total', done: userData.totalStars >= 100 },
                        { icon: '🏆', title: 'Level 10 Hero', desc: 'Reached Kingdom Level 10', done: userData.level >= 10 },
                    ].map((m) => (
                        <div key={m.title} className="p-4 flex items-center gap-4">
                            ...
                            {m.done && <Check className="w-5 h-5 text-emerald-500" strokeWidth={3} />}
                        </div>
                    ))}
                </div>
            </div>
...
// ─── ALERTS TAB ────────────────────────────────────────────────────
function ParentAlertsTab({ userData }: any) {
    const alerts = [
        { icon: '🌙', color: 'bg-indigo-100 text-indigo-600', title: 'Missed PM Brush', time: 'Yesterday', desc: `${userData.name || 'Hero'} did not log their evening brushing session.` },
        { icon: '🔥', color: 'bg-orange-100 text-orange-600', title: 'Streak Milestone!', time: `${userData.currentStreak} Days`, desc: `${userData.name || 'Hero'} is on a ${userData.currentStreak}-day streak! Celebrate this!` },
        { icon: '⬆️', color: 'bg-purple-100 text-purple-600', title: 'Level Up!', time: 'Today', desc: `${userData.name || 'Hero'} reached Level ${userData.level} in the Tooth Kingdom!` },
        { icon: '🦷', color: 'bg-emerald-100 text-emerald-600', title: 'Health Check', time: 'Weekly', desc: `Enamel health is at ${userData.enamelHealth}%. ${userData.enamelHealth > 70 ? 'Excellent condition!' : 'Consider a dental checkup.'}` },
    ];
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden divide-y divide-gray-50">
                {alerts.map((a) => (
                    <div key={a.title} className="p-5 flex items-start gap-4">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl flex-shrink-0 ${a.color}`}>{a.icon}</div>
                        <div className="flex-1">
...
export function ParentDashboardScreen({ navigateTo, userData }: ScreenProps) {
    ...
    return (
        <div className="h-full bg-gradient-to-b from-purple-50 to-white flex flex-col overflow-hidden relative">

            {/* ── HEADER ── */}
            <div className="flex-none bg-gradient-to-br from-purple-500 to-purple-600 text-white px-5 pt-5 pb-6 z-50 shadow-xl border-b border-purple-400/30">
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar px-5 py-6">
                {activeTab === 'home' && <ParentHomeTab userData={userData} />}
                {activeTab === 'progress' && <ParentProgressTab userData={userData} />}
                {activeTab === 'alerts' && <ParentAlertsTab userData={userData} />}
                <div className="pb-24" />
            </div>
                    ...
                    <div className="glass-card border-t border-white/20 px-5 py-4 flex justify-around items-center rounded-[2.5rem] shadow-[0_-8px_32px_rgba(0,0,0,0.1)]">
                        {[
                            ...
                            <button key={id} onClick={() => setActiveTab(id as any)} className="flex flex-col items-center gap-1 group">
                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${activeTab === id ? 'bg-purple-600 shadow-lg' : 'hover:bg-purple-50'}`}>
