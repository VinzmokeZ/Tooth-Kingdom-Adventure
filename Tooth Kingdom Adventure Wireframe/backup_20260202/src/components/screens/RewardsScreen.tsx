...
export function RewardsScreen({ navigateTo, userData }: ScreenProps) {
  const [activeTab, setActiveTab] = useState<'all' | 'unlocked'>('all');

  const filteredRewards = activeTab === 'unlocked' 
          ...
          {filteredRewards.map((reward) => (
            <div
              ...
              className={`bg-white rounded-3xl p-5 shadow-md transition-all ${
                reward.unlocked ? 'hover:shadow-lg' : 'opacity-75'
              }`}
            >
              {/* Reward image */}
              ...
              {/* Action */}
              {reward.unlocked ? (
                <button className={`w-full h-9 rounded-xl text-sm font-medium transition-all ${
                  reward.equipped
                ...
                <div className="flex items-center justify-center gap-1 h-9 bg-gray-100 rounded-xl">
                  <Star className="w-4 h-4 text-amber-500" />
                  <span className="text-sm font-bold text-gray-600">{reward.stars}</span>
                </div>
              )}
            </div>
          ))}
