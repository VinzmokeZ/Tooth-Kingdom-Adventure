...
import { ArrowLeft, BookOpen, Video, FileText, Download, Play, Star, Users, Baby, GraduationCap, Search, Sparkles, Bot, X } from 'lucide-react';
...
export function LearningResourcesScreen({ navigateTo }: ScreenProps) {
  ...
  const handleAskAI = () => {
    ...
    setTimeout(() => {
      setIsTyping(false);
      setAIResponse("Based on the latest dental research, children should brush for at least 2 minutes twice a day. For heroes like you, I recommend focusing on the back molars where sugar monsters love to hide! 🦷✨");
    }, 1500);
  };
          ...
          {showAIAssistant && (
            <div className="bg-gradient-to-r from-cyan-500 to-blue-500 rounded-3xl shadow-2xl p-1 animate-slideDown">
              <div className="bg-white rounded-[22px] p-6">
                ...
                {aiResponse && (
                  <div className="bg-cyan-50 border-2 border-cyan-100 rounded-2xl p-4 mb-4 animate-fadeIn">
                    <p className="text-sm text-gray-800 leading-relaxed italic">"{aiResponse}"</p>
                  </div>
                )}
                    ...
                    <button
                      onClick={() => {
                        if (resource.type === 'video') {
                          ...
                          window.open('https://www.mouthhealthy.org/en/az-topics/b/brushing-your-teeth', '_blank');
                        } else {
                          window.open('https://www.colgate.com/en-us/oral-health/kids-oral-care', '_blank');
                        }
                      }}
                      className="w-12 h-12 bg-gradient-to-br from-purple-500 to-pink-500 text-white rounded-xl flex items-center justify-center hover:scale-110 transition-transform shadow-lg flex-shrink-0"
                    >
